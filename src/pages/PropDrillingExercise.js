...
const RootComponent = (props) => {
  ...
  function addProductToCart(product) {
    const newProductsList = cart.products.map((item) => {
    ...
    setCart({ products: newProductsList, totalPrice: newTotalPrice });
  }
  ...
  function removeProductFromCart(product) {
    ...
    const newProductList = cart.products.map((item) => {
      if (product.id === item.id && item.qty > 0) {
        item.qty -= 1;
        item.price -= product.price;
